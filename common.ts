...
import { readableStreamToString, BrexWebhooksError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://accounts-api.brex.com/oauth2/default/v1/token"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BrexWebhooksError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BrexWebhooksError(e, e.response?.data)
                }
                if (innerError instanceof BrexWebhooksError) {
                    // Got "BrexWebhooksError" from the above try block
                    throw innerError;
                }
